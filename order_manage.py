...
def cancel_order(order_id):
    # Get session dict (set this after login)
    session = st.session_state.get("session", {})  # Use session_state for session info
    api_session_key = session.get("api_session_key", "")
    uid = session.get("uid", "")
    actid = session.get("actid", "")
...
def show():
    ...
    col_widths = [0.7, 1.3, 1.2, 0.8, 0.7, 1, 0.8, 0.9, 0.9, 0.9, 0.9, 1, 1.3, 0.8, 0.7, 1.2, 1.1]

    # Get API session key from session dict
